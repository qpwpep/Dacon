...
def _fit_major_maps(tr_df: pd.DataFrame, cfg: CFG) -> MajorMaps:
    ...
    def _fit_one(col: str) -> Tuple[Dict[str, int], set]:
        if col not in tr_df.columns:
            return {}, set()
        s = tr_df[col].fillna("").astype("string").str.strip()
        s = s.replace({"": "MISSING"})
        vc = s.value_counts(dropna=False)
        freq = {str(k): int(v) for k, v in vc.to_dict().items()}
        top = set(vc.head(cfg.major_topk).index.astype(str).tolist())
    ...
    if ("major1_1" in tr_df.columns) or ("major1_2" in tr_df.columns):
        m1 = tr_df.get("major1_1", pd.Series(["UNK"] * len(tr_df), index=tr_df.index)).fillna("").astype("string").str.strip()
        m2 = tr_df.get("major1_2", pd.Series(["UNK"] * len(tr_df), index=tr_df.index)).fillna("").astype("string").str.strip()
...
def _apply_major_maps(df: pd.DataFrame, maps: MajorMaps, cfg: CFG) -> None:
    ...
    def _apply_one(col: str, prefix: str, freq: Dict[str, int], top: set) -> None:
        ...
        s = df[col].fillna("").astype("string").str.strip().replace({"": "MISSING"})
        s_str = s.astype(str)
...
def main() -> None:
    ...
    # feature importance (folds may have different feature sets)
    ...
    # -----------------------------
    # 4) K-Fold Training (Stage-1 fold-local fit/apply)
    # -----------------------------
    ...
    for fold, (tr_idx, va_idx) in enumerate(splitter.split(np.zeros(len(train_base)), y_full), start=1):
        ...
        # Validation wrong samples tracking (fold threshold = cfg.decision_threshold)
        ...
        # Feature importance (fold-specific feature set)
        if cfg.save_feature_importance:
            ...
            if fi.shape[0] == len(feature_names):
                fold_fi_dicts.append({name: float(val) for name, val in zip(feature_names, fi.tolist())})
            else:
                print(f"[WARN] feature importance length mismatch: got {fi.shape[0]}, expected {len(feature_names)}. Skip this fold.")

    # -----------------------------
    # 5) OOF summary + threshold tuning
    # -----------------------------
    if is_binary:
    ...
    if is_binary and (not cfg.submit_proba):
        best_thr, best_f1 = _best_threshold_by_f1(y_full.to_numpy(), oof_pred)
        tuned_thr = float(best_thr)
        print(f"[OOF] best_thr={tuned_thr:.3f} best_F1={best_f1:.4f} (was thr={cfg.decision_threshold:.3f})")

    # -----------------------------
    # 6) Save submission (if sample submission exists; else fallback)
    # -----------------------------
    ...
    if (not cfg.submit_proba) and is_binary:
        ...
        out2 = cfg.output_path.replace(".csv", f"_thr{tuned_thr:.3f}.csv")
        try:
            submission2.to_csv(out2, index=False)
        except Exception as e:
            print(f"[WARN] failed to save tuned submission to {out2}: {e}")
